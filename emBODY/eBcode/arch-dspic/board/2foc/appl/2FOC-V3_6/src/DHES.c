...
unsigned char DHESRead()
{
    ...
    unsigned char port_read = 0 ; 
    
    if (MotorConfig.swapBC)
        port_read = (PORTBbits.RB3<<2) | (PORTBbits.RB9<<1) | (PORTBbits.RB8);
    else
        port_read = (PORTBbits.RB3<<2) | (PORTBbits.RB8<<1) | (PORTBbits.RB9);
    ...
    */
    ...
    else
    {
        ...
        dhes_position = dhes_steps_per_rev ? __builtin_divsd(((long)dhes_steps)<<16,dhes_steps_per_rev) : 0;
    }
    /*
    if (transition == IMPOSSIBLE)
    {
...
int DHESAngle()
{
    return (60*(DHESSector()-1) + 30 + dhes_offset) % 360;
}